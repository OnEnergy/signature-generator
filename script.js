...
const logoAssetUrl = new URL("resources/logo2.png", publicBaseUrl).href;
const fallbackLogoDataUrl = buildFallbackLogoDataUrl();
...
function buildSignatureHtml(data) {
    ...
    const nativeWidth = 1160;
    const nativeHeight = 368;
    const targetWidth = 820;
    const aspectPercent = (nativeHeight / nativeWidth) * 100; // keep full 3 rows visible
    const scale = targetWidth / nativeWidth;
    const paddingLeft = 360 * scale;
    const paddingTop = 32 * scale;
    const paddingRight = 28 * scale;
    const paddingBottom = 24 * scale;

    return `
<div class="signature-export" style="position:relative; display:inline-block; width:100%; max-width:${targetWidth}px; font-family:'Univers Next Pro','Helvetica Neue',Helvetica,Arial,sans-serif; color:#000000;">
    <div style="position:relative; width:100%; padding-top:${aspectPercent}%; border-radius:18px; overflow:hidden; background: transparent;">
        <div style="position:absolute; inset:0; background:url('${logoSrc}') center/100% 100% no-repeat; border-radius:18px;"></div>
        <div style="position:absolute; inset:0; padding:${paddingTop}px ${paddingRight}px ${paddingBottom}px ${paddingLeft}px; box-sizing:border-box; display:flex; flex-direction:column; gap:${8 * scale}px; justify-content:flex-start; align-items:flex-start;">
            <div style="font-size:${24 * scale}px; font-weight:600; line-height:1; margin-bottom:${0 * scale}px;">${escapeHtml(data.name)}</div>
            <div style="font-size:${18 * scale}px; line-height:1.25; color:#6c6c6c;">${escapeHtml(data.title)}</div>
            <div style="margin-top:${10 * scale}px; font-size:${16 * scale}px; line-height:1.45;">
